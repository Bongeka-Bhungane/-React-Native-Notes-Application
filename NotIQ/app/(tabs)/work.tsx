...
export default function WorkScreen() {
  ...
  const [filteredNotes, setFilteredNotes] = useState(workNotes);
  const [sortBy, setSortBy] = useState<"dateAdded" | "dateEdited">("dateAdded");
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("desc");
  const [filterVisible, setFilterVisible] = useState(false);

  useEffect(() => {
    const work = notes.filter((note) => note.category.toLowerCase() === "work");
    const sorted = sortNotes(sortBy, sortOrder).filter(
      (note) => note.category.toLowerCase() === "work"
    );
    setWorkNotes(sorted);
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>
              {searchQuery
                ? "No work notes found"
                : "No work notes yet. Create your first work note!"}
            </Text>
          </View>
        }
      />
