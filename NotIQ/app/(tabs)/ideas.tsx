...
export default function IdeasScreen() {
  ...
  const [ideasNotes, setIdeasNotes] = useState(notes);
  const [searchQuery, setSearchQuery] = useState("");
  const [filteredNotes, setFilteredNotes] = useState(ideasNotes);
  ...
  useEffect(() => {
    const ideas = notes.filter(
      (note) => note.category.toLowerCase() === "ideas"
    );
    const sorted = sortNotes(sortBy, sortOrder).filter(
      (note) => note.category.toLowerCase() === "ideas"
    );
    setIdeasNotes(sorted);
